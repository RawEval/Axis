"""connector.slack.recent_activity — time-windowed feed of Slack messages.

Reads from the local activity_events cache (kept fresh by SlackSyncWorker
in connector-manager). The FreshenBeforeRead mixin runs a synchronous
/tools/slack/freshen call if cache is older than 60s — invisible to the user.
"""
...
@dataclass
class _SlackRecentActivity(FreshenBeforeRead):
    name: str = "connector.slack.recent_activity"
    description: str = (
        "List Slack messages posted in a time window. Use for prompts like "
        "'what was discussed in Slack today' or 'what did the team talk about this week'. "
        ...
        *,
        ...
        async with db.raw.acquire() as conn:
            sql = """
                ...
                WHERE user_id = $1::uuid AND source = 'slack' AND occurred_at >= $2
            """
        ...
        citations = [
            Citation(
                source_type=f"slack_{r['event_type']}",
                provider="slack",
                ref_id=str(r["id"]),
                title=r["title"],
                actor=r.get("actor"),
        ...
        return CapabilityResult(
            summary=f"found {len(events)} slack events since {inputs['since']}",
            content={"events": events, "freshness": freshness.model_dump()},
            citations=citations,
        )


CAPABILITY = _SlackRecentActivity()
