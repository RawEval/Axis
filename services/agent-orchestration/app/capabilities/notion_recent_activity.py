...
Reads from the local activity_events cache (kept fresh by NotionSyncWorker
in connector-manager). The FreshenBeforeRead mixin runs a synchronous
/tools/notion/freshen call if cache is older than 60s — invisible to the user.
"""
...
class _NotionRecentActivity(FreshenBeforeRead):
    ...
    description: str = (
        "List Notion pages edited in a time window. Use for prompts like "
        "'what happened in Notion today' or 'what did I edit in Notion this week'. "
