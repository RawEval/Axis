"""connector.github.recent_activity — time-windowed feed of GitHub events.

Reads from the local activity_events cache (kept fresh by GitHubSyncWorker
in connector-manager). The FreshenBeforeRead mixin runs a synchronous
/tools/github/freshen call if cache is older than 60s — invisible to the user.
"""
...
@dataclass
class _GitHubRecentActivity(FreshenBeforeRead):
    name: str = "connector.github.recent_activity"
    description: str = (
        "List GitHub activity (pushes, PRs, issues, comments, releases) in a time window. "
        "Use for prompts like 'what happened on GitHub today' or 'what PRs moved this week'. "
        ...
        *,
        ...
        async with db.raw.acquire() as conn:
            sql = """
                ...
                WHERE user_id = $1::uuid AND source = 'github' AND occurred_at >= $2
            """
        ...
        citations = [
            Citation(
                source_type=f"github_{r['event_type']}",
                provider="github",
                ref_id=str(r["id"]),
                title=r["title"],
                actor=r.get("actor"),
        ...
        return CapabilityResult(
            summary=f"found {len(events)} github events since {inputs['since']}",
            content={"events": events, "freshness": freshness.model_dump()},
            citations=citations,
        )


CAPABILITY = _GitHubRecentActivity()
