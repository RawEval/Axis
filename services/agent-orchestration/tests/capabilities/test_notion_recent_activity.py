"""Tests for connector.notion.recent_activity capability.
...
async def test_returns_seeded_notion_event_with_freshness(db_pool, monkeypatch):
    user_id = uuid4()
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            await conn.execute(
                """INSERT INTO activity_events
                   ...
                   VALUES ($1, 'notion', 'page_edited', 'page-test',
        ...
        assert "1 notion" in result.summary.lower() or "found 1" in result.summary.lower()
        assert isinstance(result.content, dict)
        assert "events" in result.content
        assert len(result.content["events"]) == 1
        assert result.content["events"][0]["title"] == "Test Page"
        assert "freshness" in result.content
        assert result.content["freshness"]["sync_status"] == "ok"
        assert result.content["freshness"]["source"] == "notion"
    finally:
...
async def test_empty_when_no_events_in_window(db_pool, monkeypatch):
    ...
    try:
        ...
        # Empty events but ok freshness — agent can say "I checked Notion just now, no activity"
        assert result.content["freshness"]["sync_status"] == "ok"
    finally:
        await _cleanup(db_pool, user_id)
