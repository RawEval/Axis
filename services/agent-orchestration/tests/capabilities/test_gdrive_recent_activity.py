"""Tests for connector.gdrive.recent_activity capability.
...
async def test_returns_seeded_gdrive_event_with_freshness(db_pool, monkeypatch):
    user_id = uuid4()
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO connector_sync_state
                   ...
                   VALUES ($1, 'gdrive', NOW(), 'ok', '{}'::jsonb)""",
                user_id,
            )
            await conn.execute(
                """INSERT INTO activity_events
                   (user_id, source, event_type, external_id, title, snippet, occurred_at, raw_ref)
                   VALUES ($1, 'gdrive', 'file_modified', 'file-abc',
                           'Q3 Plan.docx', 'application/vnd.google-apps.document',
        ...
        from app.capabilities.gdrive_recent_activity import CAPABILITY
        ...
        assert "1 gdrive" in result.summary.lower() or "found 1" in result.summary.lower()
        assert isinstance(result.content, dict)
        assert "events" in result.content
        assert len(result.content["events"]) == 1
        assert result.content["events"][0]["title"] == "Q3 Plan.docx"
        assert "freshness" in result.content
        assert result.content["freshness"]["sync_status"] == "ok"
        assert result.content["freshness"]["source"] == "gdrive"
    finally:
...
async def test_empty_when_no_events_in_window(db_pool, monkeypatch):
    ...
    try:
        ...
        # Empty events but ok freshness — agent can say "I checked Drive just now, no activity"
        assert result.content["freshness"]["sync_status"] == "ok"
    finally:
        await _cleanup(db_pool, user_id)
