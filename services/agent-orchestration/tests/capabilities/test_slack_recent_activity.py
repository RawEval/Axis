"""Tests for connector.slack.recent_activity capability.
...
async def test_returns_seeded_slack_event_with_freshness(db_pool, monkeypatch):
    user_id = uuid4()
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO connector_sync_state
                   ...
                   VALUES ($1, 'slack', NOW(), 'ok', '{}'::jsonb)""",
                user_id,
            )
            await conn.execute(
                """INSERT INTO activity_events
                   (user_id, source, event_type, external_id, title, snippet, occurred_at, raw_ref)
                   VALUES ($1, 'slack', 'message_posted', 'C1:1745020800.000100',
                           'Hello from the test suite', NULL, NOW() - INTERVAL '2 minutes', '{}'::jsonb)""",
                user_id,
            )
        ...
        from app.capabilities.slack_recent_activity import CAPABILITY
        ...
        assert "1 slack" in result.summary.lower() or "found 1" in result.summary.lower()
        assert isinstance(result.content, dict)
        assert "events" in result.content
        assert len(result.content["events"]) == 1
        assert result.content["events"][0]["title"] == "Hello from the test suite"
        assert "freshness" in result.content
        assert result.content["freshness"]["sync_status"] == "ok"
        assert result.content["freshness"]["source"] == "slack"
    finally:
...
async def test_empty_when_no_events_in_window(db_pool, monkeypatch):
    ...
    try:
        ...
        # Empty events but ok freshness — agent can say "I checked Slack just now, no activity"
        assert result.content["freshness"]["sync_status"] == "ok"
    finally:
        await _cleanup(db_pool, user_id)
