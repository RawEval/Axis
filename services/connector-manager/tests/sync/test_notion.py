"""End-to-end tests for NotionSyncWorker — mocks Notion API via respx,
...
async def _seed_notion_connector(db_pool, user_id):
    """Insert a user + connector row so the worker has something to sync. The
    auth_token_encrypted column is bytea but we patch decrypt_token, so the
    bytes value doesn't matter."""
...
async def test_freshen_writes_activity_event_and_records_success(db_pool, fake_notion_page):
    ...
    user_id = uuid4()
    await _seed_notion_connector(db_pool, user_id)
    try:
        ...
        with patch("app.sync.notion.decrypt_token", return_value="dummy_token"):
            result = await NotionSyncWorker().freshen(user_id, _pool=db_pool)
        ...
        async with db_pool.acquire() as conn:
            events = await conn.fetch(
                ...
                "WHERE user_id = $1 AND source = 'notion'", user_id,
            )
            assert len(events) == 1
            assert events[0]["external_id"] == "page-abc"
            assert events[0]["title"] == "Test Page"
...
async def test_freshen_is_idempotent_on_repeated_call(db_pool, fake_notion_page):
    ...
    try:
        ...
        with patch("app.sync.notion.decrypt_token", return_value="t"):
            await NotionSyncWorker().freshen(user_id, _pool=db_pool)
            await NotionSyncWorker().freshen(user_id, _pool=db_pool)
...
async def test_freshen_no_op_when_no_connector(db_pool):
    """User with no Notion connector → freshen returns ok with 0 rows."""
    from app.sync.notion import NotionSyncWorker
